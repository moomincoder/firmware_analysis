...
normal_regex="pass|user|admin|root|ftp|nfs|smb|ssh|snmp|pop3|imap|smtp|HTB\{|C1\{|flag\{|((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}"
...
web_regex="(\b25[0-5]|\b2[0-4][0-9]|\b[01]?[0-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}ftp|nfs|smb|ssh|snmp|pop3|imap|smtp"
...
function binwalking {
	inputFile=$1 
	binwalk -eM $inputFile
}

function grepping_time {
	folder_path=$1
	rg -is -p "$regex" $folder_path
	# I wanted some regex to find bas64, but this one needs to be more restrictive
	# rg -is -p "(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?"
}
